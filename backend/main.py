...
# Unificar caminho do SQLite usando caminho absoluto relativo ao diretório deste script
import os
...
class InfoSalaModel(Base):
    ...
    sala_id = Column(String, nullable=False)  # ID numérico gerado pelo Free Fire
...
def executar_migracoes():
    try:
        with engine.begin() as connection:
            ...
            # Adiciona senha_hash se não existir
            if "senha_hash" not in colunas:
                print("Migração: Adicionando coluna 'senha_hash' à tabela 'jogadores'.")
            ...
            # Adiciona saldo se não existir
            if "saldo" not in colunas:
                print("Migração: Adicionando coluna 'saldo' à tabela 'jogadores'.")
            ...
            if "is_admin" not in colunas:
                print("Migração: Adicionando coluna 'is_admin' à tabela 'jogadores'.")
            ...
            print("Migrações do banco de dados verificadas e executadas com sucesso.")
    except Exception as e:
        print(f"Erro crítico durante migração do banco de dados: {e}")
...
def obter_usuario_atual(db, x_user_id: Optional[str] = Header(None)) -> JogadorModel:
    if not x_user_id:
        raise HTTPException(
            ...
            detail="Usuário não autenticado (x-user-id ausente)."
        )
    try:
...
# ROTAS DE JOGADORES E AUTENTICAÇÃO
...
def processar_deposito(id: int, dados: ProcessarDepositoInput, x_user_id: str = Header(...), db: Session = Depends(get_db)):
    ...
    if not req:
        raise HTTPException(status_code=404, detail="Requisição de depósito não encontrada.")
        
    if req.status != "pendente":
        raise HTTPException(status_code=400, detail="Esta requisição já foi processada.")
    ...
    db.commit()
    return {"message": f"Depósito {id} processado com status: {dados.status}."}
...
def inscrever_queda(numero_queda: int, x_user_id: str = Header(...), db: Session = Depends(get_db)):
    ...
    if inscricao_existente:
        return {"message": "Jogador já está inscrito nesta queda.", "inscrito": True}
    ...
    if count_inscritos >= 48:
        raise HTTPException(status_code=400, detail="Esta queda já atingiu o limite de 48 jogadores.")
        
    # 3. Verificar saldo (R$ 2,00)
    if jogador.saldo < 2.0:
        raise HTTPException(status_code=400, detail="Saldo insuficiente. Você precisa de no mínimo R$ 2,00 para se inscrever.")
...
def obter_status_queda(numero_queda: int, x_user_id: Optional[str] = Header(None), db: Session = Depends(get_db)):
    ...
    # 2. Verificar se o jogador logado está inscrito
    esta_inscrito = False
    if x_user_id:
        try:
            user_id = int(x_user_id)
    ...
    # 3. Verificar se dados de sala estão liberados
...
def cancelar_reembolsar_queda(numero_queda: int, x_user_id: str = Header(...), db: Session = Depends(get_db)):
    ...
    # Também apagar dados de sala se existirem
...
async def processar_ocr_queda(numero_queda: int, file: UploadFile = File(...), x_user_id: str = Header(...), db: Session = Depends(get_db)):
    ...
    if not api_key:
        raise HTTPException(status_code=400, detail="Chave de API do Gemini não configurada no backend.")
    ...
    try:
        ...
        # Instrução para leitura do placar do Free Fire
        prompt = (
            ...
            "- jogador_nick (o nickname visível no placar)\n"
            "- colocacao (a posição final dele, um número inteiro de 1 a 52)\n"
            "- abates (a quantidade de kills/eliminações, número inteiro de 0 a 50)\n\n"
            "Você deve responder APENAS com um array JSON válido, contendo objetos com os campos 'jogador_nick', 'colocacao' e 'abates'.\n"
            "Não adicione nenhuma explicação, markdown ou caracteres extras fora do JSON.\n"
            "Exemplo de saída:\n"
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Gemini retornou um formato JSON inválido: {response.text}"
        )
    except Exception as e:
        raise HTTPException(
            status_code=500,
...
def obter_sala(numero_queda: int, db: Session = Depends(get_db)):
    sala = db.query(InfoSalaModel).filter(InfoSalaModel.numero_queda == numero_queda).first()
    if not sala:
        raise HTTPException(status_code=404, detail="Dados da sala para esta queda ainda não foram liberados.")
    return sala
...
def registrar_queda(dados: RegistroQuedaBatch, db: Session = Depends(get_db)):
    ...
    for res in dados.resultados:
        ...
        if not jogador:
            raise HTTPException(status_code=404, detail=f"Jogador ID {res.jogador_id} não existe.")
        ...
        if registro_duplicado:
            raise HTTPException(status_code=400, detail=f"Jogador ID {res.jogador_id} já possui resultado registrado na Queda {dados.numero_queda}.")
    ...
    db.commit()
    return {"message": f"Resultados da Queda {dados.numero_queda} salvos e premiações pagas!"}

# Função interna para obter classificação
...
def criar_jogador_ferramenta(nome: str, nick: str) -> str:
    ...
    Parâmetros:
    - nome: Nome completo do jogador (ex: João da Silva)
    ...
    try:
        ...
        if db_jogador:
            return f"Erro: O jogador com nick '{nick}' já está cadastrado no sistema."
...
def liberar_sala_ferramenta(numero_queda: int, sala_id: str, senha: str) -> str:
    """
    Cadastra ou atualiza o ID da sala e a Senha para uma queda (partida) específica.
    Parâmetros:
    - numero_queda: O número da partida/queda (ex: 1, 2, 3)
    - sala_id: O ID numérico da sala personalizada gerado pelo Free Fire (ex: 88392)
...
def registrar_resultado_individual_ferramenta(numero_queda: int, jogador_nick: str, colocacao: int, abates: int) -> str:
    """
    Registra a pontuação e abates obtidos por um jogador em uma queda específica.
    Parâmetros:
    - numero_queda: O número da queda em que a partida ocorreu (ex: 1, 2)
    - jogador_nick: O nickname exato do jogador participante (ex: Nobru)
    - colocacao: A posição final do jogador na partida (entre 1 e 48)
    - abates: A quantidade de eliminações (abates/kills) feitas por esse jogador
    """
    db = SessionLocal()
    try:
        jogador = db.query(JogadorModel).filter(JogadorModel.nick == jogador_nick).first()
        if not jogador:
            return f"Erro: O jogador com o nick '{jogador_nick}' não está cadastrado no campeonato."
            
        registro_duplicado = db.query(QuedaModel).filter(
            QuedaModel.numero_queda == numero_queda, QuedaModel.jogador_id == jogador.id
        ).first()
        if registro_duplicado:
            return f"Erro: O jogador '{jogador_nick}' já possui pontuação cadastrada para a Queda {numero_queda}."
            
        # Creditar prêmio se houver colocação premiada
        ...
        db.add(nova_pontuacao)
        db.commit()
        return f"Sucesso: Pontuação gravada para '{jogador_nick}' na Queda {numero_queda}. Colocação: {colocacao}º lugar, Abates: {abates}."
    except Exception as e:
        db.rollback()
        return f"Erro ao registrar pontuação individual: {str(e)}"
    finally:
        db.close()
...
def listar_jogadores_cadastrados_ferramenta() -> str:
    ...
    Útil para consultar IDs, nomes e nicks cadastrados no banco de dados.
    """
    import json
    db = SessionLocal()
    try:
...
def obter_classificacao_atual_ferramenta() -> str:
    """
    Retorna a classificação geral atual do campeonato (leaderboard), contendo os pontos,
    ...
    try:
        ...
        return f"Erro ao obter classificação: {str(e)}"
    finally:
        db.close()

def cadastrar_jogadores_lote_ferramenta(jogadores_json: str) -> str:
    """
    Cadastra múltiplos competidores (jogadores) de uma única vez (em lote).
    Parâmetros:
    ...
    try:
        ...
        for jog in dados_jogadores:
            ...
            if not nome or not nick:
                erros.append(f"Jogador inválido (nome ou nick ausente): {jog}")
                continue
                
            db_jogador = db.query(JogadorModel).filter(JogadorModel.nick == nick).first()
            if db_jogador:
                erros.append(f"Nick '{nick}' já cadastrado.")
                continue
...
def registrar_resultados_lote_ferramenta(numero_queda: int, resultados_json: str) -> str:
    """
    Registra a pontuação e abates de múltiplos jogadores (ou de toda a partida) para uma queda específica em lote.
    Parâmetros:
    - numero_queda: O número da queda/partida (ex: 1, 2)
    ...
    try:
        ...
        for res in dados_resultados:
            ...
            if not nick or colocacao is None:
                erros.append(f"Resultado inválido (nick ou colocação ausente): {res}")
                continue
            ...
            try:
                ...
                erros.append(f"Colocação ou abates inválidos para o nick '{nick}': {res}")
                continue
                
            if colocacao < 1 or colocacao > 52:
                erros.append(f"Colocação inválida para o nick '{nick}' (deve ser entre 1 e 52): {colocacao}")
                continue
                
            jogador = db.query(JogadorModel).filter(JogadorModel.nick == nick).first()
            if not jogador:
                erros.append(f"Nick '{nick}' não encontrado.")
                continue
            ...
            if registro_duplicado:
                erros.append(f"O jogador '{nick}' já pontuou na Queda {numero_queda}.")
                continue
            ...
            db.add(nova_pontuacao)
            sucessos.append(f"'{nick}' em {colocacao}º lugar ({abates} abates)")
...
def processar_comando_agente(dados: AgenteComandoInput):
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="A biblioteca 'google-generativeai' não está instalada no servidor. Por favor, instale executando 'pip install google-generativeai'."
        )
    ...
    if not api_key:
        raise HTTPException(
            status_code=400,
            detail="Chave de API do Gemini não configurada. Configure a variável de ambiente GEMINI_API_KEY ou envie sua chave pelo painel de chat."
        )
    ...
    try:
        ...
        model = genai.GenerativeModel(
            ...
            tools=ferramentas,
            system_instruction=(
                "Você é o assistente virtual inteligente administrador do campeonato de Free Fire (Modo Solo).\n"
                "Seu dever é receber comandos do organizador e chamar a ferramenta apropriada correspondente.\n"
                "Você tem o superpoder de gerenciar todo o campeonato por texto natural:\n"
                "1. Cadastrar competidores (individuais usando criar_jogador_ferramenta ou múltiplos usando cadastrar_jogadores_lote_ferramenta).\n"
                ...
                "4. Consultar a tabela de classificação/leaderboard (usando obter_classificacao_atual_ferramenta) para responder dúvidas de posições.\n"
                "5. Registrar resultados/abates de partidas (individual usando registrar_resultado_individual_ferramenta ou múltiplos jogadores em lote de uma vez só usando registrar_resultados_lote_ferramenta).\n\n"
                "ATENÇÃO COM RESULTADOS DE FIM DE JOGO:\n"
                "Se o organizador enviar uma lista de resultados (ex: tabela copiando e colando, ou frase listando vários jogadores), faça o seguinte:\n"
                "a) Chame listar_jogadores_cadastrados_ferramenta para buscar os nicks corretos se necessário e verificar se existem.\n"
                "b) Extraia de cada linha o nick, colocação (ex: 1º, 2º) e abates.\n"
                "c) Chame a ferramenta registrar_resultados_lote_ferramenta fornecendo o número da queda e os dados no formato JSON exigido (uma lista de objetos com 'jogador_nick', 'colocacao' e 'abates').\n\n"
                "Sempre responda de maneira profissional, prestativa e em português brasileiro.\n"
                "Ao terminar uma ação com sucesso, informe ao organizador de forma clara e resumida os detalhes da operação executada no banco de dados."
            )
        )
