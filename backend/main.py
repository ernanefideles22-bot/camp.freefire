...
# ==========================================
# CARREGAMENTO DAS VARIÃÂÃÂÃÂÃÂVEIS DE AMBIENTE (.env)
# ==========================================
def carregar_env():
...
# Unificar caminho do SQLite usando caminho absoluto relativo ao diretÃÂÃÂÃÂÃÂ³rio deste script
import os
...
class InfoSalaModel(Base):
    ...
    sala_id = Column(String, nullable=False)  # ID numÃÂÃÂÃÂÃÂ©rico gerado pelo Free Fire
...
def executar_migracoes():
    try:
        with engine.begin() as connection:
            ...
            # Adiciona senha_hash se nÃÂÃÂÃÂÃÂ£o existir
            if "senha_hash" not in colunas:
                print("MigraÃÂÃÂÃÂÃÂ§ÃÂÃÂÃÂÃÂ£o: Adicionando coluna 'senha_hash' ÃÂÃÂÃÂÃÂ  tabela 'jogadores'.")
            ...
            # Adiciona saldo se nÃÂÃÂÃÂÃÂ£o existir
            if "saldo" not in colunas:
                print("MigraÃÂÃÂÃÂÃÂ§ÃÂÃÂÃÂÃÂ£o: Adicionando coluna 'saldo' ÃÂÃÂÃÂÃÂ  tabela 'jogadores'.")
                connection.execute("ALTER TABLE jogadores ADD COLUMN saldo FLOAT DEFAULT 0.0")
                
            # Adiciona is_admin se nÃÂÃÂÃÂÃÂ£o existir
            if "is_admin" not in colunas:
                print("MigraÃÂÃÂÃÂÃÂ§ÃÂÃÂÃÂÃÂ£o: Adicionando coluna 'is_admin' ÃÂÃÂÃÂÃÂ  tabela 'jogadores'.")
            ...
            print("MigraÃÂÃÂÃÂÃÂ§ÃÂÃÂÃÂÃÂµes do banco de dados verificadas e executadas com sucesso.")
    except Exception as e:
        print(f"Erro crÃÂÃÂÃÂÃÂ­tico durante migraÃÂÃÂÃÂÃÂ§ÃÂÃÂÃÂÃÂ£o do banco de dados: {e}")
...
def seed_admin():
    ...
    try:
        _adm = _db.query(JogadorModel).filter(JogadorModel.nick == 'fideles').first()
        if not _adm:
            _h = hashlib.sha256('13032020'.encode('utf-8')).hexdigest()
            _adm = JogadorModel(nome='Fideles', nick='fideles', senha_hash=_h, saldo=0.0, is_admin=True)
            _db.add(_adm)
            _db.commit()
            print('[SEED] Admin criado: nick=admin senha=13032020')
...
# ==========================================
# SCHEMAS DE VALIDAÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂO (PYDANTIC)
# ==========================================
...
# ==========================================
# MOTOR DE PONTUAÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂO
# ==========================================
...
# ==========================================
# CONFIGURAÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂO DO MIDDLEWARE CORS
# ==========================================
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
# ==========================================
# FUNÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂES AUXILIARES / UTILITÃÂÃÂÃÂÃÂRIAS
# ==========================================
def get_db():
    db = SessionLocal()
    try:
...
def obter_usuario_atual(db, x_user_id: Optional[str] = Header(None)) -> JogadorModel:
    if not x_user_id:
        raise HTTPException(
            ...
            detail="UsuÃÂÃÂÃÂÃÂ¡rio nÃÂÃÂÃÂÃÂ£o autenticado (x-user-id ausente)."
        )
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="ID de usuÃÂÃÂÃÂÃÂ¡rio invÃÂÃÂÃÂÃÂ¡lido."
        )
    jogador = db.query(JogadorModel).filter(JogadorModel.id == user_id).first()
    if not jogador:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="UsuÃÂÃÂÃÂÃÂ¡rio nÃÂÃÂÃÂÃÂ£o encontrado."
        )
    return jogador

# ROTAS DE JOGADORES E AUTENTICAÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂO
...
def cadastrar_usuario(jogador: JogadorCreate, db: Session = Depends(get_db)):
    ...
    if db_jogador:
        raise HTTPException(status_code=400, detail="Este Nick jÃÂÃÂÃÂÃÂ¡ estÃÂÃÂÃÂÃÂ¡ cadastrado.")
...
# ROTAS DE CARTEIRA / DEPÃÂÃÂÃÂÃÂSITOS
...
def processar_deposito(id: int, dados: ProcessarDepositoInput, x_user_id: str = Header(...), db: Session = Depends(get_db)):
    ...
    if not req:
        raise HTTPException(status_code=404, detail="RequisiÃÂÃÂÃÂÃÂ§ÃÂÃÂÃÂÃÂ£o de depÃÂÃÂÃÂÃÂ³sito nÃÂÃÂÃÂÃÂ£o encontrada.")
        
    if req.status != "pendente":
        raise HTTPException(status_code=400, detail="Esta requisiÃÂÃÂÃÂÃÂ§ÃÂÃÂÃÂÃÂ£o jÃÂÃÂÃÂÃÂ¡ foi processada.")
        
    if dados.status not in ["aprovado", "rejeitado"]:
        raise HTTPException(status_code=400, detail="Status invÃÂÃÂÃÂÃÂ¡lido. Use 'aprovado' ou 'rejeitado'.")
    ...
    db.commit()
    return {"message": f"DepÃÂÃÂÃÂÃÂ³sito {id} processado com status: {dados.status}."}
...
# ROTAS DE INSCRIÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂES
...
def inscrever_queda(numero_queda: int, x_user_id: str = Header(...), db: Session = Depends(get_db)):
    ...
    # 1. Verificar se jÃÂÃÂÃÂÃÂ¡ estÃÂÃÂÃÂÃÂ¡ inscrito
    ...
    if inscricao_existente:
        return {"message": "Jogador jÃÂÃÂÃÂÃÂ¡ estÃÂÃÂÃÂÃÂ¡ inscrito nesta queda.", "inscrito": True}
    ...
    if count_inscritos >= 48:
        raise HTTPException(status_code=400, detail="Esta queda jÃÂÃÂÃÂÃÂ¡ atingiu o limite de 48 jogadores.")
        
    # 3. Verificar saldo (R$ 2,00)
    if jogador.saldo < 2.0:
        raise HTTPException(status_code=400, detail="Saldo insuficiente. VocÃÂÃÂÃÂÃÂª precisa de no mÃÂÃÂÃÂÃÂ­nimo R$ 2,00 para se inscrever.")
        
    # 4. Debitar saldo
    jogador.saldo -= 2.0
    
    # 5. Criar inscriÃÂÃÂÃÂÃÂ§ÃÂÃÂÃÂÃÂ£o
    ...
    return {
        "message": "InscriÃÂÃÂÃÂÃÂ§ÃÂÃÂÃÂÃÂ£o realizada com sucesso!",
        "inscrito": True,
...
def obter_status_queda(numero_queda: int, x_user_id: Optional[str] = Header(None), db: Session = Depends(get_db)):
    ...
    # 2. Verificar se o jogador logado estÃÂÃÂÃÂÃÂ¡ inscrito
    esta_inscrito = False
    if x_user_id:
        try:
            user_id = int(x_user_id)
    ...
    # 3. Verificar se dados de sala estÃÂÃÂÃÂÃÂ£o liberados
...
def cancelar_reembolsar_queda(numero_queda: int, x_user_id: str = Header(...), db: Session = Depends(get_db)):
    ...
    # TambÃÂÃÂÃÂÃÂ©m apagar dados de sala se existirem
...
async def processar_ocr_queda(numero_queda: int, file: UploadFile = File(...), x_user_id: str = Header(...), db: Session = Depends(get_db)):
    ...
    if not api_key:
        raise HTTPException(status_code=400, detail="Chave de API do Gemini nÃÂÃÂÃÂÃÂ£o configurada no backend.")
    ...
    try:
        ...
        # InstruÃÂÃÂÃÂÃÂ§ÃÂÃÂÃÂÃÂ£o para leitura do placar do Free Fire
        prompt = (
            ...
            "- jogador_nick (o nickname visÃÂÃÂÃÂÃÂ­vel no placar)\n"
            "- colocacao (a posiÃÂÃÂÃÂÃÂ§ÃÂÃÂÃÂÃÂ£o final dele, um nÃÂÃÂÃÂÃÂºmero inteiro de 1 a 52)\n"
            "- abates (a quantidade de kills/eliminaÃÂÃÂÃÂÃÂ§ÃÂÃÂÃÂÃÂµes, nÃÂÃÂÃÂÃÂºmero inteiro de 0 a 50)\n\n"
            "VocÃÂÃÂÃÂÃÂª deve responder APENAS com um array JSON vÃÂÃÂÃÂÃÂ¡lido, contendo objetos com os campos 'jogador_nick', 'colocacao' e 'abates'.\n"
            "NÃÂÃÂÃÂÃÂ£o adicione nenhuma explicaÃÂÃÂÃÂÃÂ§ÃÂÃÂÃÂÃÂ£o, markdown ou caracteres extras fora do JSON.\n"
            "Exemplo de saÃÂÃÂÃÂÃÂ­da:\n"
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Gemini retornou um formato JSON invÃÂÃÂÃÂÃÂ¡lido: {response.text}"
        )
    except Exception as e:
        raise HTTPException(
            status_code=500,
...
def obter_sala(numero_queda: int, db: Session = Depends(get_db)):
    sala = db.query(InfoSalaModel).filter(InfoSalaModel.numero_queda == numero_queda).first()
    if not sala:
        raise HTTPException(status_code=404, detail="Dados da sala para esta queda ainda nÃÂÃÂÃÂÃÂ£o foram liberados.")
    return sala

# ROTAS DE PONTUAÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂO
...
def registrar_queda(dados: RegistroQuedaBatch, db: Session = Depends(get_db)):
    ...
    for res in dados.resultados:
        ...
        if not jogador:
            raise HTTPException(status_code=404, detail=f"Jogador ID {res.jogador_id} nÃÂÃÂÃÂÃÂ£o existe.")
        ...
        if registro_duplicado:
            raise HTTPException(status_code=400, detail=f"Jogador ID {res.jogador_id} jÃÂÃÂÃÂÃÂ¡ possui resultado registrado na Queda {dados.numero_queda}.")
    ...
    for res in dados.resultados:
        jogador = db.query(JogadorModel).filter(JogadorModel.id == res.jogador_id).first()
        # Creditar prÃÂÃÂÃÂÃÂªmio se houver
    ...
    db.commit()
    return {"message": f"Resultados da Queda {dados.numero_queda} salvos e premiaÃÂÃÂÃÂÃÂ§ÃÂÃÂÃÂÃÂµes pagas!"}

# FunÃÂÃÂÃÂÃÂ§ÃÂÃÂÃÂÃÂ£o interna para obter classificaÃÂÃÂÃÂÃÂ§ÃÂÃÂÃÂÃÂ£o
...
def obter_historico_jogador(nick: str, db: Session = Depends(get_db)):
    ...
    if not jogador:
        raise HTTPException(status_code=404, detail="Jogador nÃÂÃÂÃÂÃÂ£o encontrado.")
...
def criar_jogador_ferramenta(nome: str, nick: str) -> str:
    ...
    ParÃÂÃÂÃÂÃÂ¢metros:
    - nome: Nome completo do jogador (ex: JoÃÂÃÂÃÂÃÂ£o da Silva)
    ...
    try:
        ...
        if db_jogador:
            return f"Erro: O jogador com nick '{nick}' jÃÂÃÂÃÂÃÂ¡ estÃÂÃÂÃÂÃÂ¡ cadastrado no sistema."
...
def liberar_sala_ferramenta(numero_queda: int, sala_id: str, senha: str) -> str:
    """
    Cadastra ou atualiza o ID da sala e a Senha para uma queda (partida) especÃÂÃÂÃÂÃÂ­fica.
    ParÃÂÃÂÃÂÃÂ¢metros:
    - numero_queda: O nÃÂÃÂÃÂÃÂºmero da partida/queda (ex: 1, 2, 3)
    - sala_id: O ID numÃÂÃÂÃÂÃÂ©rico da sala personalizada gerado pelo Free Fire (ex: 88392)
...
def registrar_resultado_individual_ferramenta(numero_queda: int, jogador_nick: str, colocacao: int, abates: int) -> str:
    """
    Registra a pontuaÃÂÃÂÃÂÃÂ§ÃÂÃÂÃÂÃÂ£o e abates obtidos por um jogador em uma queda especÃÂÃÂÃÂÃÂ­fica.
    ParÃÂÃÂÃÂÃÂ¢metros:
    - numero_queda: O nÃÂÃÂÃÂÃÂºmero da queda em que a partida ocorreu (ex: 1, 2)
    - jogador_nick: O nickname exato do jogador participante (ex: Nobru)
    - colocacao: A posiÃÂÃÂÃÂÃÂ§ÃÂÃÂÃÂÃÂ£o final do jogador na partida (entre 1 e 48)
    - abates: A quantidade de eliminaÃÂÃÂÃÂÃÂ§ÃÂÃÂÃÂÃÂµes (abates/kills) feitas por esse jogador
    """
    db = SessionLocal()
    try:
        jogador = db.query(JogadorModel).filter(JogadorModel.nick == jogador_nick).first()
        if not jogador:
            return f"Erro: O jogador com o nick '{jogador_nick}' nÃÂÃÂÃÂÃÂ£o estÃÂÃÂÃÂÃÂ¡ cadastrado no campeonato."
            
        registro_duplicado = db.query(QuedaModel).filter(
            QuedaModel.numero_queda == numero_queda, QuedaModel.jogador_id == jogador.id
        ).first()
        if registro_duplicado:
            return f"Erro: O jogador '{jogador_nick}' jÃÂÃÂÃÂÃÂ¡ possui pontuaÃÂÃÂÃÂÃÂ§ÃÂÃÂÃÂÃÂ£o cadastrada para a Queda {numero_queda}."
            
        # Creditar prÃÂÃÂÃÂÃÂªmio se houver colocaÃÂÃÂÃÂÃÂ§ÃÂÃÂÃÂÃÂ£o premiada
        ...
        db.add(nova_pontuacao)
        db.commit()
        return f"Sucesso: PontuaÃÂÃÂÃÂÃÂ§ÃÂÃÂÃÂÃÂ£o gravada para '{jogador_nick}' na Queda {numero_queda}. ColocaÃÂÃÂÃÂÃÂ§ÃÂÃÂÃÂÃÂ£o: {colocacao}ÃÂÃÂÃÂÃÂº lugar, Abates: {abates}."
    except Exception as e:
        db.rollback()
        return f"Erro ao registrar pontuaÃÂÃÂÃÂÃÂ§ÃÂÃÂÃÂÃÂ£o individual: {str(e)}"
    finally:
        db.close()
...
def listar_jogadores_cadastrados_ferramenta() -> str:
    ...
    ÃÂÃÂÃÂÃÂtil para consultar IDs, nomes e nicks cadastrados no banco de dados.
    """
    import json
    db = SessionLocal()
    try:
...
def obter_classificacao_atual_ferramenta() -> str:
    """
    Retorna a classificaÃÂÃÂÃÂÃÂ§ÃÂÃÂÃÂÃÂ£o geral atual do campeonato (leaderboard), contendo os pontos,
    ...
    try:
        ...
        return f"Erro ao obter classificaÃÂÃÂÃÂÃÂ§ÃÂÃÂÃÂÃÂ£o: {str(e)}"
    finally:
        db.close()

def cadastrar_jogadores_lote_ferramenta(jogadores_json: str) -> str:
    """
    Cadastra mÃÂÃÂÃÂÃÂºltiplos competidores (jogadores) de uma ÃÂÃÂÃÂÃÂºnica vez (em lote).
    ParÃÂÃÂÃÂÃÂ¢metros:
    ...
    try:
        ...
        for jog in dados_jogadores:
            ...
            if not nome or not nick:
                erros.append(f"Jogador invÃÂÃÂÃÂÃÂ¡lido (nome ou nick ausente): {jog}")
                continue
                
            db_jogador = db.query(JogadorModel).filter(JogadorModel.nick == nick).first()
            if db_jogador:
                erros.append(f"Nick '{nick}' jÃÂÃÂÃÂÃÂ¡ cadastrado.")
                continue
...
def registrar_resultados_lote_ferramenta(numero_queda: int, resultados_json: str) -> str:
    """
    Registra a pontuaÃÂÃÂÃÂÃÂ§ÃÂÃÂÃÂÃÂ£o e abates de mÃÂÃÂÃÂÃÂºltiplos jogadores (ou de toda a partida) para uma queda especÃÂÃÂÃÂÃÂ­fica em lote.
    ParÃÂÃÂÃÂÃÂ¢metros:
    - numero_queda: O nÃÂÃÂÃÂÃÂºmero da queda/partida (ex: 1, 2)
    ...
    try:
        ...
        for res in dados_resultados:
            ...
            if not nick or colocacao is None:
                erros.append(f"Resultado invÃÂÃÂÃÂÃÂ¡lido (nick ou colocaÃÂÃÂÃÂÃÂ§ÃÂÃÂÃÂÃÂ£o ausente): {res}")
                continue
            ...
            try:
                ...
                erros.append(f"ColocaÃÂÃÂÃÂÃÂ§ÃÂÃÂÃÂÃÂ£o ou abates invÃÂÃÂÃÂÃÂ¡lidos para o nick '{nick}': {res}")
                continue
                
            if colocacao < 1 or colocacao > 52:
                erros.append(f"ColocaÃÂÃÂÃÂÃÂ§ÃÂÃÂÃÂÃÂ£o invÃÂÃÂÃÂÃÂ¡lida para o nick '{nick}' (deve ser entre 1 e 52): {colocacao}")
                continue
                
            jogador = db.query(JogadorModel).filter(JogadorModel.nick == nick).first()
            if not jogador:
                erros.append(f"Nick '{nick}' nÃÂÃÂÃÂÃÂ£o encontrado.")
                continue
            ...
            if registro_duplicado:
                erros.append(f"O jogador '{nick}' jÃÂÃÂÃÂÃÂ¡ pontuou na Queda {numero_queda}.")
                continue
            ...
            db.add(nova_pontuacao)
            sucessos.append(f"'{nick}' em {colocacao}ÃÂÃÂÃÂÃÂº lugar ({abates} abates)")
...
def processar_comando_agente(dados: AgenteComandoInput):
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="A biblioteca 'google-generativeai' nÃÂÃÂÃÂÃÂ£o estÃÂÃÂÃÂÃÂ¡ instalada no servidor. Por favor, instale executando 'pip install google-generativeai'."
        )
    ...
    if not api_key:
        raise HTTPException(
            status_code=400,
            detail="Chave de API do Gemini nÃÂÃÂÃÂÃÂ£o configurada. Configure a variÃÂÃÂÃÂÃÂ¡vel de ambiente GEMINI_API_KEY ou envie sua chave pelo painel de chat."
        )
    ...
    try:
        ...
        model = genai.GenerativeModel(
            ...
            tools=ferramentas,
            system_instruction=(
                "VocÃÂÃÂÃÂÃÂª ÃÂÃÂÃÂÃÂ© o assistente virtual inteligente administrador do campeonato de Free Fire (Modo Solo).\n"
                "Seu dever ÃÂÃÂÃÂÃÂ© receber comandos do organizador e chamar a ferramenta apropriada correspondente.\n"
                "VocÃÂÃÂÃÂÃÂª tem o superpoder de gerenciar todo o campeonato por texto natural:\n"
                "1. Cadastrar competidores (individuais usando criar_jogador_ferramenta ou mÃÂÃÂÃÂÃÂºltiplos usando cadastrar_jogadores_lote_ferramenta).\n"
                ...
                "4. Consultar a tabela de classificaÃÂÃÂÃÂÃÂ§ÃÂÃÂÃÂÃÂ£o/leaderboard (usando obter_classificacao_atual_ferramenta) para responder dÃÂÃÂÃÂÃÂºvidas de posiÃÂÃÂÃÂÃÂ§ÃÂÃÂÃÂÃÂµes.\n"
                "5. Registrar resultados/abates de partidas (individual usando registrar_resultado_individual_ferramenta ou mÃÂÃÂÃÂÃÂºltiplos jogadores em lote de uma vez sÃÂÃÂÃÂÃÂ³ usando registrar_resultados_lote_ferramenta).\n\n"
                "ATENÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂO COM RESULTADOS DE FIM DE JOGO:\n"
                "Se o organizador enviar uma lista de resultados (ex: tabela copiando e colando, ou frase listando vÃÂÃÂÃÂÃÂ¡rios jogadores), faÃÂÃÂÃÂÃÂ§a o seguinte:\n"
                "a) Chame listar_jogadores_cadastrados_ferramenta para buscar os nicks corretos se necessÃÂÃÂÃÂÃÂ¡rio e verificar se existem.\n"
                "b) Extraia de cada linha o nick, colocaÃÂÃÂÃÂÃÂ§ÃÂÃÂÃÂÃÂ£o (ex: 1ÃÂÃÂÃÂÃÂº, 2ÃÂÃÂÃÂÃÂº) e abates.\n"
                "c) Chame a ferramenta registrar_resultados_lote_ferramenta fornecendo o nÃÂÃÂÃÂÃÂºmero da queda e os dados no formato JSON exigido (uma lista de objetos com 'jogador_nick', 'colocacao' e 'abates').\n\n"
                "Sempre responda de maneira profissional, prestativa e em portuguÃÂÃÂÃÂÃÂªs brasileiro.\n"
                "Ao terminar uma aÃÂÃÂÃÂÃÂ§ÃÂÃÂÃÂÃÂ£o com sucesso, informe ao organizador de forma clara e resumida os detalhes da operaÃÂÃÂÃÂÃÂ§ÃÂÃÂÃÂÃÂ£o executada no banco de dados."
            )
        )
