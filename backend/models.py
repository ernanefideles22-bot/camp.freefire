...
# Torneio pago isolado das quedas avulsas e do evento bonus.
class EventoPagoModel(Base):
    __tablename__ = 'eventos_pagos'
    id: Mapped[int] = mapped_column(Integer, primary_key=True, index=True)
    nome: Mapped[str] = mapped_column(String, nullable=False)
    status: Mapped[str] = mapped_column(String, default='inscricao', index=True)
    min_jogadores: Mapped[int] = mapped_column(Integer, default=2)
...
class InscricaoPagaModel(Base):
    __tablename__ = 'inscricoes_pagas'
    id: Mapped[int] = mapped_column(Integer, primary_key=True, index=True)
    evento_id: Mapped[int] = mapped_column(ForeignKey('eventos_pagos.id'), nullable=False, index=True)
    jogador_id: Mapped[int] = mapped_column(ForeignKey('jogadores.id'), nullable=False, index=True)
    criado_em: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utcnow)

class ResultadoPagoModel(Base):
    __tablename__ = 'resultados_pagos'
    id: Mapped[int] = mapped_column(Integer, primary_key=True, index=True)
    evento_id: Mapped[int] = mapped_column(ForeignKey('eventos_pagos.id'), nullable=False, index=True)
...
class PagamentoPagoModel(Base):
    __tablename__ = 'pagamentos_pagos'
    id: Mapped[int] = mapped_column(Integer, primary_key=True, index=True)
    evento_id: Mapped[int] = mapped_column(ForeignKey('eventos_pagos.id'), nullable=False, index=True)
    jogador_id: Mapped[int] = mapped_column(ForeignKey('jogadores.id'), nullable=False, index=True)
    colocacao_final: Mapped[int] = mapped_column(Integer, nullable=False)
