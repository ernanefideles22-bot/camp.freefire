...
function App() {
  ...
  // Helper to trigger custom toasts from children components
  ...
  const handleLogout = () => {
    ...
    handleAddToast('info', 'Sessão encerrada', 'Você saiu da sua conta.');
  };

  const tabClasses = (tabName: TabType) => {
    const base =
      'flex items-center gap-2 px-5 py-3 rounded-xl font-bold text-sm transition-all duration-200 cursor-pointer ';
    if (activeTab === tabName) {
      return (
        base +
        'bg-gradient-to-r from-primary to-primary-dark text-white shadow-[0_0_15px_rgba(139,92,246,0.3)]'
      );
    }
  ...
  return (
    <div className="min-h-screen bg-body-bg text-zinc-100 flex flex-col selection:bg-primary selection:text-white relative overflow-x-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-2">
            <button
              ...
              className={tabClasses('leaderboard')}
            >
            ...
            <button
              ...
              className={tabClasses('player_portal')}
            >
          ...
          {/* User Section / Session Quick Indicator */}
      ...
      {/* Navigation - Mobile Floating Bar */}
      <div className="md:hidden fixed bottom-4 left-4 right-4 z-40 bg-panel-bg/95 border border-zinc-800 rounded-2xl p-1.5 flex justify-around shadow-2xl backdrop-blur-md">
        <button
          onClick={() => setActiveTab('leaderboard')}
          className={`flex flex-col items-center gap-1 py-2 px-3 rounded-xl flex-1 transition-all ${
            activeTab === 'leaderboard' ? 'text-primary font-black' : 'text-zinc-500 font-medium'
          }`}
        >
        ...
        <button
          onClick={() => setActiveTab('player_portal')}
          className={`flex flex-col items-center gap-1 py-2 px-3 rounded-xl flex-1 transition-all ${
            activeTab === 'player_portal' ? 'text-primary font-black' : 'text-zinc-500 font-medium'
          }`}
        >
        ...
        <button
          onClick={() => setActiveTab('admin')}
          className={`flex flex-col items-center gap-1 py-2 px-3 rounded-xl flex-1 transition-all ${
            activeTab === 'admin' ? 'text-primary font-black' : 'text-zinc-500 font-medium'
          }`}
        >
            ...
            currentUser ? (
              ...
              <AuthPortal
                onAuthSuccess={(user) => {
                  ...
                  if (user.is_admin) {
                    setActiveTab('admin');
                    handleAddToast('info', 'Painel de Administrador', 'Você foi direcionado para o painel de administração.');
                  }
                }}
                addToast={(title, desc, type) => handleAddToast(type || 'info', title, desc)}
              />
            )
          )}
              ...
              currentUser.is_admin ? (
                ...
                <div className="max-w-xl mx-auto mt-12 bg-panel-bg rounded-2xl border border-zinc-800 p-8 shadow-2xl text-center">
                  ...
                  <p className="text-zinc-400 text-sm leading-relaxed mb-6">
                    Esta área é exclusiva para administradores do campeonato. Se você é um organizador, faça login com a conta administrativa.
                  </p>
                  <button
                    onClick={handleLogout}
              ...
              <AuthPortal
                onAuthSuccess={(user) => {
                  ...
                  if (user.is_admin) {
                    ...
                    handleAddToast('warning', 'Redirecionado', 'Você foi direcionado para o Portal do Jogador pois sua conta não é administradora.');
                  }
                }}
                addToast={(title, desc, type) => handleAddToast(type || 'info', title, desc)}
              />
