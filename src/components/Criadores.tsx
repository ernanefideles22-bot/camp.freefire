...
import { CalendarDays, Crown, Link, Play, RefreshCw, Share2, Shield, Trophy, Upload, Users } from 'lucide-react';
...
function OperacaoEvento({ evento, aoAtualizar, onAddToast }: { evento: CampeonatoCriador; aoAtualizar: () => Promise<void>; onAddToast: Props['onAddToast'] }) {
  ...
  const salvarResultado = () => {
    const resultados = (evento.inscritos_jogadores ?? []).map(jogador => ({
      jogador_id: jogador.id,
      colocacao: Number(linhas[jogador.id]?.colocacao),
      abates: Number(linhas[jogador.id]?.abates || 0),
    }));
  ...
  const lerOcr = async (arquivo: File) => {
    ...
    try {
      ...
      for (const item of resposta.resultados ?? []) {
        if (item.jogador_id) novo[item.jogador_id] = { colocacao: String(item.colocacao || ''), abates: String(item.abates || 0) };
      }
  ...
  return <div className="mt-4 rounded-xl border border-primary/25 bg-primary/5 p-4 space-y-4">
    ...
    <div className="space-y-2">{(evento.inscritos_jogadores ?? []).map(jogador => <div key={jogador.id} className="grid grid-cols-[1fr_76px_76px] gap-2 items-center"><span className="flex min-w-0 items-center gap-2 truncate text-xs font-bold text-white">{jogador.guilda?.logo_url && <img src={jogador.guilda.logo_url} alt="" className="h-6 w-6 rounded object-cover border border-primary/35" />}<span className="truncate">{jogador.nick}{jogador.guilda?.nome && <small className="ml-1 text-[9px] text-primary">· {jogador.guilda.nome}</small>}</span></span><input type="number" min="1" placeholder="Pos." value={linhas[jogador.id]?.colocacao ?? ''} onChange={e => setLinhas(atual => ({ ...atual, [jogador.id]: { ...atual[jogador.id], colocacao: e.target.value } }))} className="ff-input w-full px-2 py-1.5 text-xs text-white" /><input type="number" min="0" placeholder="Abates" value={linhas[jogador.id]?.abates ?? ''} onChange={e => setLinhas(atual => ({ ...atual, [jogador.id]: { ...atual[jogador.id], abates: e.target.value } }))} className="ff-input w-full px-2 py-1.5 text-xs text-white" /></div>)}</div>
    ...
    : <>
      ...
      <section className="ff-card ff-creator-create p-5 space-y-4">
        ...
        <div className="grid md:grid-cols-3 gap-3">
          ...
          <label><span>VAGAS</span><input value={vagas} onChange={e => setVagas(e.target.value)} type="number" min="2" placeholder="48" className="ff-input w-full px-3 py-2 text-sm text-white" /></label>
          ...
          <label><span>ENTRADA (R$)</span><input value={taxa} onChange={e => setTaxa(e.target.value)} type="number" min="0" step="0.01" placeholder="3,00" className="ff-input w-full px-3 py-2 text-sm text-white" /></label>
          <div className="ff-creator-info"><Users className="w-4 h-4" /><span>Inscrição por jogador ou equipe, conforme a modalidade.</span></div>
        </div>
