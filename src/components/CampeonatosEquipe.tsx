...
import { Check, ChevronDown, ChevronUp, Shield, Swords, Users, Trophy, RefreshCw } from 'lucide-react';
...
export function CampeonatosEquipe({ currentUser, onAddToast }: Props) {
  ...
  const abrirInscricao = async (eventoId: number) => {
    setAbrir(eventoId); setNomeEquipe(''); setNicks(''); setMembrosSelecionados([]); setListaAberta(false);
  ...
  const inscrever = async (ev: CampeonatoEquipe) => {
    ...
    try {
      ...
      setAbrir(null); setNomeEquipe(''); setNicks(''); setMembrosSelecionados([]); await carregar();
