...
import { Check, ChevronDown, ChevronUp, Crown, Crosshair, Flame, ImagePlus, Medal, Shield, Swords, Users, Trophy, RefreshCw, Share2 } from 'lucide-react';
...
function PlacarArena({ itens, evento, onAddToast }: { itens: CampeonatoEquipe['placar']; evento: CampeonatoEquipe; onAddToast: Props['onAddToast'] }) {
  ...
  const compartilhar = async (escopo: 'top3' | 'completo') => {
    const linhas = escopo === 'top3' ? itens.slice(0, 3) : itens;
    const titulo = escopo === 'top3' ? 'TOP 3 DA ARENA' : 'RANKING COMPLETO DA ARENA';
    const texto = [
      '🏆 FLOWFIRE CHAMPIONS',
      evento.nome.toUpperCase(),
      `${formato(evento)} · ${evento.total_rodadas ?? 3} rodada(s)`,
      '',
      titulo,
      ...linhas.map(item => `${item.posicao}º ${item.equipe} — ${item.pontos} pts · ${item.abates} abates`),
      '',
      'Jogue. Suba no ranking. Domine a arena.',
    ].join('\n');
    try {
      if (navigator.share) await navigator.share({ title: `${evento.nome} — ${titulo}`, text: texto, url: window.location.href });
      else {
        await navigator.clipboard.writeText(`${texto}\n${window.location.href}`);
        onAddToast('success', 'Ranking copiado', `${escopo === 'top3' ? 'O Top 3' : 'O ranking completo'} está pronto para enviar.`);
      }
    } catch { /* compartilhamento cancelado pelo jogador */ }
  };
  return <section className="ff-event-ranking">
    <div className="ff-event-ranking-hero"><div><span className="ff-kicker">Classificação em tempo real</span><h4><Trophy /> MURAL DA ARENA</h4><p>Quem domina a queda sobe no ranking.</p></div><div className="ff-event-ranking-actions"><span className="ff-event-ranking-live"><i />PLACAR ATUALIZADO</span><button type="button" onClick={() => compartilhar('top3')} className="ff-event-ranking-share"><Trophy />Top 3</button><button type="button" onClick={() => compartilhar('completo')} className="ff-event-ranking-share is-full"><Share2 />Ranking completo</button></div></div>
  ...
  return <div className="max-w-4xl mx-auto space-y-4">
    ...
    {campeonatos.map(ev => {
      ...
      return <section key={ev.id} className="rounded-2xl border border-zinc-800 bg-panel-bg p-5 space-y-4">
        ...
        <PlacarArena itens={ev.placar} evento={ev} onAddToast={onAddToast} />
