...
function ChaveCSAdmin({ evento, busy, onConfirmar }: { evento: CampeonatoEquipe; busy: boolean; onConfirmar: (confrontoId: number, vencedorId: number, abatesA: number, abatesB: number) => void }) {
  const [abates, setAbates] = useState<Record<number, { a: string; b: string }>>({});
  const fases = Array.from(new Set((evento.confrontos_cs ?? []).map(confronto => confronto.fase)));
  if (!fases.length) return <div className="rounded-xl border border-amber-500/25 bg-amber-500/5 p-4 text-xs text-amber-200">A chave CS está sendo gerada. Atualize em alguns segundos.</div>;
  return <div className="space-y-4 border-t border-zinc-800 pt-5"><div><span className="ff-kicker">Chave eliminatória CS</span><p className="mt-1 text-xs text-zinc-400">Cada duelo vale uma vaga na próxima fase. Confirme a equipe vencedora e os abates do confronto.</p></div><div className="space-y-4">{fases.map(fase => <section key={fase} className="rounded-xl border border-primary/25 bg-zinc-950/35 p-3"><div className="mb-3 flex items-center gap-2"><span className="grid h-6 w-6 place-items-center rounded bg-primary text-xs font-black text-white">{fase}</span><b className="text-xs uppercase tracking-wider text-white">Fase {fase}</b></div><div className="grid gap-3 lg:grid-cols-2">{(evento.confrontos_cs ?? []).filter(confronto => confronto.fase === fase).map(confronto => { const a = confronto.equipe_a; const b = confronto.equipe_b; const valores = abates[confronto.id] ?? { a: String(confronto.abates_a || ''), b: String(confronto.abates_b || '') }; const vencedor = confronto.vencedor_id; return <div key={confronto.id} className="rounded-xl border border-zinc-800 bg-zinc-950/80 p-3 space-y-2"><div className="flex items-center justify-between text-[10px] uppercase tracking-wider text-zinc-500"><span>Duelo {confronto.ordem}</span><span className={confronto.status === 'aguardando' ? 'text-amber-400' : 'text-emerald-400'}>{confronto.status === 'aguardando' ? 'Aguardando resultado' : confronto.status === 'bye' ? 'Avanço automático' : 'Finalizado'}</span></div><div className={`flex items-center justify-between gap-2 rounded-lg px-2.5 py-2 ${vencedor === a?.id ? 'bg-emerald-500/10 border border-emerald-500/30' : 'bg-zinc-900/70'}`}><span className="min-w-0 font-bold text-sm text-white truncate">{a?.nome}</span><input disabled={confronto.status !== 'aguardando'} type="number" min="0" placeholder="Abates" value={valores.a} onChange={e => setAbates(atual => ({ ...atual, [confronto.id]: { ...valores, a: e.target.value } }))} className="w-20 rounded border border-zinc-700 bg-zinc-950 px-2 py-1 text-right text-xs text-white disabled:opacity-50" /></div><div className="text-center text-[10px] font-black text-primary">VS</div>{b ? <div className={`flex items-center justify-between gap-2 rounded-lg px-2.5 py-2 ${vencedor === b.id ? 'bg-emerald-500/10 border border-emerald-500/30' : 'bg-zinc-900/70'}`}><span className="min-w-0 font-bold text-sm text-white truncate">{b.nome}</span><input disabled={confronto.status !== 'aguardando'} type="number" min="0" placeholder="Abates" value={valores.b} onChange={e => setAbates(atual => ({ ...atual, [confronto.id]: { ...valores, b: e.target.value } }))} className="w-20 rounded border border-zinc-700 bg-zinc-950 px-2 py-1 text-right text-xs text-white disabled:opacity-50" /></div> : <div className="rounded-lg border border-dashed border-zinc-700 px-2.5 py-2 text-center text-xs text-zinc-500">Avança sem adversário</div>}{confronto.status === 'aguardando' && a && b && <div className="grid grid-cols-2 gap-2 pt-1"><button disabled={busy} onClick={() => onConfirmar(confronto.id, a.id, Number(valores.a || 0), Number(valores.b || 0))} className="rounded-lg bg-primary px-2 py-2 text-xs font-bold text-white disabled:opacity-40 cursor-pointer">Vitória {a.nome}</button><button disabled={busy} onClick={() => onConfirmar(confronto.id, b.id, Number(valores.a || 0), Number(valores.b || 0))} className="rounded-lg border border-primary/50 px-2 py-2 text-xs font-bold text-primary disabled:opacity-40 cursor-pointer">Vitória {b.nome}</button></div>}</div>; })}</div></section>)}</div></div>;
}
...
export function AdminEquipes({ onAddToast }: Props) {
  ...
  const confirmarConfrontoCs = (confrontoId: number, vencedorId: number, abatesA: number, abatesB: number) => {
    if (!evento) return;
    executar(() => apiService.confirmarConfrontoCs(evento.id, confrontoId, vencedorId, abatesA, abatesB), 'Confronto confirmado');
  };
  ...
  return <div className="space-y-6">
    <div className="ff-card p-5 space-y-3"><div className="flex items-center justify-between gap-3"><h2 className="text-sm font-bold text-white flex items-center gap-2"><Swords className="w-4 h-4 text-primary" />Campeonatos por equipe</h2><button onClick={carregar} className="p-1.5 text-zinc-500 hover:text-white rounded-lg cursor-pointer" title="Atualizar"><RefreshCw className="w-3.5 h-3.5" /></button></div><p className="text-xs text-zinc-400">No CS, o sistema sorteia os confrontos e avança os vencedores até a final. BR continua com placar por rodada.</p>{evento && <div className="flex flex-wrap gap-2">{evento.premios.map((premio, i) => <span key={i} className="text-[11px] font-bold px-2.5 py-1 rounded-lg bg-zinc-950 border border-zinc-800 text-zinc-300">{i + 1}o <b className="text-emerald-400">{brl(premio)}</b></span>)}</div>}</div>
    ...
    {evento && <div className="ff-card p-5 space-y-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="text-sm font-black text-white">{evento.nome} <span className="text-zinc-500">#{evento.id}</span></h3><span className="text-[10px] font-bold uppercase tracking-wider text-primary">{formato} - {evento.tamanho_equipe} por equipe - {evento.status.replace('_', ' ')}</span></div><div className="flex items-center gap-2 bg-zinc-950 border border-zinc-800 px-3 py-1.5 rounded-xl"><Users className="w-4 h-4 text-primary" /><span className="text-sm font-black text-white">{equipes.length}</span><span className="text-[10px] text-zinc-500">/ min. {evento.min_equipes}</span></div></div>{evento.status === 'inscricao' && <button onClick={compartilharLink} className="flex items-center gap-2 rounded-lg border border-primary/40 bg-primary/10 px-3 py-2 text-xs font-bold text-primary hover:bg-primary/15 cursor-pointer"><Share2 className="w-3.5 h-3.5" />Compartilhar campeonato</button>}
      ...
      {evento.tipo === 'br' && evento.status === 'em_andamento' && <div className="rounded-xl border border-primary/35 bg-primary/5 p-4 space-y-3"><div><span className="ff-kicker">Slots do lobby Free Fire</span><p className="mt-1 text-xs text-zinc-400">Cada cartão abaixo ocupa um slot. Em Duo e Squad, todos os jogadores da equipe entram juntos no mesmo número.</p></div><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{[...equipes].sort((a, b) => (a.slot_ff ?? 999) - (b.slot_ff ?? 999)).map(equipe => <div key={equipe.id} className="flex items-center gap-3 rounded-lg border border-zinc-800 bg-zinc-950/65 px-3 py-2"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-primary font-black text-white">{String(equipe.slot_ff ?? 0).padStart(2, '0')}</span><span className="min-w-0"><b className="block truncate text-xs text-white">{equipe.nome}</b><small className="block truncate text-[10px] text-zinc-500">{equipe.membros.map(membro => membro.nick).join(' · ')}</small></span></div>)}</div></div>}
      {evento.status === 'em_andamento' && evento.tipo === 'cs_4x4' && <><div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-amber-500/30 bg-amber-500/5 p-3"><p className="text-xs text-amber-200">Iniciou a chave por engano? Enquanto nenhum duelo foi confirmado, você pode voltar às inscrições.</p><button disabled={busy} onClick={() => window.confirm('Apagar a chave sorteada e reabrir as inscrições?') && executar(() => apiService.reabrirInscricoesCampeonatoEquipe(evento.id), 'Inscrições reabertas')} className="flex items-center gap-2 rounded-lg border border-amber-500/50 px-3 py-2 text-xs font-bold text-amber-300 hover:bg-amber-500/10 cursor-pointer disabled:opacity-40"><RotateCcw className="w-3.5 h-3.5" />Voltar às inscrições</button></div><ChaveCSAdmin evento={evento} busy={busy} onConfirmar={confirmarConfrontoCs} /></>}
      {evento.status === 'em_andamento' && evento.tipo !== 'cs_4x4' && <div className="space-y-4 border-t border-zinc-800 pt-5">
