    ...
    <div className="space-y-6">
      ...
      <div className="ff-card p-5 space-y-3">
        <div className="flex items-center justify-between gap-3">
          <h2 className="text-sm font-bold text-white flex items-center gap-2"><Gift className="w-4 h-4 text-primary" />{evento?.nome ?? (nome.trim() || 'Queda Bônus')} — {evento?.total_rodadas ?? 3} rodada(s) (entrada {brl(taxaAtual)})</h2>
          ...
          {evento.status === 'em_andamento' && (
            <div className="space-y-6">
              <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-amber-500/30 bg-amber-500/5 p-3">
                ...
                <button disabled={busy} onClick={() => window.confirm('Reabrir as inscrições deste torneio?') && guard(() => apiService.reabrirInscricoesPago(evento.id), 'Inscrições reabertas')} className="flex items-center gap-2 rounded-lg border border-amber-500/50 px-3 py-2 text-xs font-bold text-amber-300 hover:bg-amber-500/10 cursor-pointer disabled:opacity-40"><RotateCcw className="w-3.5 h-3.5" />Voltar às inscrições</button>
              </div>
              {/* Salas das 3 quedas */}
